...
namespace AdvancedSharpAdbClient
{
    ...
    public class SyncService : ISyncService, IDisposable
    {
        ...
        public void Push(List<byte[]> FileBytes, string remotePath, int permissions, DateTimeOffset timestamp, IProgress<int> progress, CancellationToken cancellationToken)
        {
            if (remotePath == null)
            {
                throw new ArgumentNullException(nameof(remotePath));
            ...
            // Try to get the total amount of bytes to transfer. This is not always possible, for example,
            // for forward-only streams.
            long totalBytesToProcess = FileBytes.Sum((byte[] x) => x.Length);
            long totalBytesRead = 0;

            // look while there is something to read
            foreach (byte[] readBytes in FileBytes)
            {
                // check if we're canceled
                cancellationToken.ThrowIfCancellationRequested();

                // read up to SYNC_DATA_MAX
                int read = readBytes.Length;
                totalBytesRead += read;

                if (readBytes.Length >= this.MaxBufferSize - 8)
                {
                    Buffer.BlockCopy(dataBytes, 0, buffer, 0, 4);
                    Buffer.BlockCopy(lengthBytes, 0, buffer, 4, 4);
                    Buffer.BlockCopy(readBytes, 0, buffer, 8, readBytes.Length);
                }
                else
                {
                    lengthBytes = BitConverter.GetBytes(read);
